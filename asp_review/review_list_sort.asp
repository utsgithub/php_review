...
<% 
	...
	If arr<>"" then
		...
		maxBound=Ubound(arr)
		for i=0 to maxBound
			sql="UPDATE review_list SET num = "&i+1&" WHERE id = "&arr(i)
			'response.Write(sql&"<br/>")
			conn.execute(sql)
		next
		response.Redirect("review_list_sort.asp?cid="&request.QueryString("cid"))
		response.End()
	end if
 %>
<%
...
%>
                <li id="<%=(Recordset1.Fields.Item("ID").Value)%>" class="list-group-item" style="cursor:move;"><span class="glyphicon glyphicon-sort"></span> <%=(Recordset1.Fields.Item("question").Value)%></li>
                <% 
...
%>
    ...
    <ul class="list-inline">
        <li> <a class="btn btn-primary f_sort">Submit</a></li>
        <li> <a class="btn btn-default" href="review_list.asp?cid=<%= request.QueryString("cid") %>">Back List</a></li>
    </ul>
...
$(".f_sort").click(function(){
		...
		url="review_list_sort.asp?arr="+arr+"&cid=<%= request.QueryString("cid") %>";
		//alert(url);
