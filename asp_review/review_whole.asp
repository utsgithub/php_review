...
Do until rs2.eof %>
    <div class="mtm">
        ...
        <div class="none"><strong><em><% = num %>:<% = (rs2.Fields.Item("question").Value) %></em></strong></div>
        <% if rs2.Fields.Item("img").Value<>"" then %>
        <div class="none">
        	<img src="images/<% = (rs2.Fields.Item("img").Value) %>" alt="..." class="img-thumbnail" style="max-width:800px;">
        </div>
        <% End If %>
        <div>
            <% If rs2.Fields.Item("Answer_draft").Value <> "" Then %>
            [<%= num_count %><%' Response.Write(left(rs("categoryname").value,10)) %>]&nbsp;<a target="_blank" href="review_edit2.asp?cid=<%= rs("id").value %>&id=<%=rs2.Fields.Item("id")%>"><% = (Replace(rs2.Fields.Item("Answer_draft").Value, Chr(10), "<br/>")) %></a>
            <% End If %>
        </div>
        <div class="none">
